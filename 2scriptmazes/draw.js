...
//theMaze = drawMaze(xSize,ySize,0,theCells,myScene);
function drawMaze(x,y,num,theCells,myScene) {

//var harch = myScene.getMeshByName("wallh_text");

	var hwall = myScene.getMeshByName("wallh_text");
	//hwall.setEnabled(false);
	var vwall = myScene.getMeshByName("wallv_text");
	//vwall.setEnabled(false);
	var varch = myScene.getMeshByName("arch_v");
	//varch.setEnabled(false);
	var harch = myScene.getMeshByName("arch_h");
	
	var theHdoor = myScene.getMeshByName("hdoor");
	var theVdoor = myScene.getMeshByName("vdoor");
...
// place doors to stairs in the multi array  of cells by switching certain values to 1 for each maze
	...
	var lCount = 0;
					...
					//console.log("last cell = " + theCells[lastrow][lastcol]);
					//console.log("first cell = " + theCells[0][0]);
					for (i = 0; i < ySize; i++){
							...
							for (k =0; k < aCell.length; k++){
								...
								case 0:
									...
									if(aCell[k] == 0){ // create solid horizontal wall at top of cell
										...
										var newHWall = harch.createInstance("harch_" +num + "_" + i + j);
										newHWall.position = new BABYLON.Vector3(hPosz, 0, vPosz+2);
										//theHdoor.position = new BABYLON.Vector3(-hPosz, 0, -vPosz+2);
										//console.log(newHWall.name + " = " + theHdoor.position);
									}
									else {
										...
										//newHWall.position = new BABYLON.Vector3(hPosz, 0, vPosz+2);
										//theHdoor.position = new BABYLON.Vector3(hPosz, 0, vPosz+2);
										//console.log("Pos = " + theHdoor.position);
								...
								case 1:
									...
									if (aCell[k] == 0){ // create a solid vertical wall at right of cell
										...
										newVWall.position = new BABYLON.Vector3(hPosz+2, 0, vPosz);
										//theVdoor.position = new BABYLON.Vector3(hPosz+2, 0, vPosz);
									}
									else {
										
										var newHWall = varch.createInstance("varch_" +num + "_" + i + j);
										newHWall.position = new BABYLON.Vector3(hPosz+2, 0, vPosz);
										theVdoor.position = new BABYLON.Vector3(hPosz+2, 0, vPosz);
										//newHWall.position = new BABYLON.Vector3(10, 4, 10);
										...
										//var newVGate = drawGates(x, y, num, newVWall, myScene);
					...
					theHdoor.position = new BABYLON.Vector3(-hPosz, 0, -vPosz+2);
					//console.log("HDoor pos = " + theHdoor.position);
...
function drawStairs (x,y,num,myScene) {
	...
	var aVGate = myScene.getMeshByName("gate_v");
	...
	var locks = myScene.getMeshByName("lock_v");
	locks.setEnabled(false);
	var aLock = locks.createInstance("lock1" +num );
	
	var aVLever = myScene.getMeshByName("lever");
...
//get all the tower parts and move them to top of last staircase
	var theTower = myScene.getMeshByName("theTower");
	theTower.position = new BABYLON.Vector3(-((x+2)*4 - 2), 16, -8);
	
	var towerDoor = myScene.getMeshByName("fdoor1");
	var dknob = myScene.getMeshByName("dknob");
	var outside = myScene.getMeshByName("outside");
	var aLight = myScene.getLightByName("Hemi");
...
/*
function drawGates(x,y,num,whichWall, myScene) {
